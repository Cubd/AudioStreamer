...
@property (assign, atomic, readwrite) AudioStreamerState state;
#if defined (USE_PREBUFFER) && USE_PREBUFFER
@property (assign, atomic, readwrite) BOOL allBufferPushed;
...
void audioRouteChangeListenerCallback(void *inUserData, AudioSessionPropertyID inPropertyID, UInt32 inPropertyValueSize, const void *inPropertyValue)
{
    ...
    if (__streamer != nil && (__streamer.isPlaying || __streamer.isIdle || __streamer.isWaiting))
    {
...
- (BOOL)isFinishing
{
	@synchronized (self)
	{
		if ((_errorCode != AS_NO_ERROR && state != AS_INITIALIZED) || ((state == AS_STOPPING || state == AS_STOPPED) && _stopReason != AS_STOPPING_TEMPORARILY))
		{
			return YES;
		}
	}
...
- (BOOL)runLoopShouldExit
{
	@synchronized(self)
	{
		if (_errorCode != AS_NO_ERROR || (state == AS_STOPPED && _stopReason != AS_STOPPING_TEMPORARILY))
		{
			return YES;
		}
	}
...
+ (NSString *)stringForErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	switch (anErrorCode)
	{
		...
		case AS_AUDIO_QUEUE_PAUSE_FAILED:
			return AS_AUDIO_QUEUE_PAUSE_FAILED_STRING;
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		[self presentAlertWithTitle:[NSString stringWithFormat:@"Fail error: %d", self.errorCode] message:[AudioStreamer stringForErrorCode:self.errorCode]];
	}
}
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
        ...
        else
        {
            ...
            {
                CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"), (__bridge CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%d", (long)seekByteOffset, fileLength - 1]);
                discontinuous = _vbr;
            }
...
//                [self presentAlertWithTitle:NSLocalizedStringFromTable(@"File Error", @"Errors", nil) message:NSLocalizedStringFromTable(@"Unable to configure network read stream.", @"Errors", nil)];
                ...
                [self presentAlertWithTitle:@"Error" message:@"Failed to set read stream auto-redirect property."];
                return NO;
            }
...
//            [self presentAlertWithTitle:NSLocalizedStringFromTable(@"File Error", @"Errors", nil) message:NSLocalizedStringFromTable(@"Unable to configure network read stream.", @"Errors", nil)];
            [self presentAlertWithTitle:@"Error" message:@"Unable to open network read stream."];
        ...
        CFReadStreamSetClient(stream, kCFStreamEventHasBytesAvailable | kCFStreamEventErrorOccurred | kCFStreamEventEndEncountered, ASReadStreamCallBack,
                              &context);
...
- (double)calculatedBitRate
{
	if (_vbr)
	{
		...
		if (_bitRate)
		{
			return (double)_bitRate;
		}
	}
	else
	{
		_bitRate = 8.0 * asbd.mSampleRate * asbd.mBytesPerPacket * asbd.mFramesPerPacket;
		return _bitRate;
	}
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		else
        {
            if (!_httpHeaders)
            {
                ...
                if (seekByteOffset == 0)
                {
                    fileLength = [[_httpHeaders objectForKey:@"Content-Length"] integerValue];
                }
            }
        }
		...
		if (!audioFileStream)
		{
            ...
            [AudioStreamer hintForMIMEType:[_httpHeaders objectForKey:@"Content-Type"]];
				...
				if (statusCode == 200)		// "OK" (this is true even for ICY)
				{
					...
					else
					{
                        ...
                        else
                        {
							...
							NSString *icyBr;
							metaInt = (NSString *) CFBridgingRelease(CFHTTPMessageCopyHeaderFieldValue(myResponse, CFSTR("Icy-Metaint")));
							contentType = (NSString *) CFBridgingRelease(CFHTTPMessageCopyHeaderFieldValue(myResponse, CFSTR("Content-Type")));
							icyBr = (NSString *) CFBridgingRelease(CFHTTPMessageCopyHeaderFieldValue(myResponse, CFSTR("icy-br")));
							/*if (contentType)
                             {
...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
		...
		if (state == AS_BUFFERING || state == AS_WAITING_FOR_DATA || state == AS_FLUSHING_EOF || (state == AS_STOPPED && _stopReason == AS_STOPPING_TEMPORARILY))
		{
			//
	...
	@synchronized(self)
	{
		...
		else if (inPropertyID == kAudioFileStreamProperty_FormatList)
		{
			...
			for (int i = 0; i * sizeof(AudioFormatListItem) < formatListSize; i += sizeof(AudioFormatListItem))
			{
                ...
                if (pasbd.mFormatID == kAudioFormatMPEG4AAC_HE_V2 && [[UIDevice currentDevice] platformHasCapability:(UIDeviceSupportsARMV7)] && kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_MIN)
                {
...
- (void)updateMetaData:(NSString *)metaData
{
	...
	NSNotification *notification = [NSNotification notificationWithName:ASUpdateMetadataNotification object:self userInfo:userInfo];
    [[NSNotificationCenter defaultCenter] postNotification:notification];
}
#endif
